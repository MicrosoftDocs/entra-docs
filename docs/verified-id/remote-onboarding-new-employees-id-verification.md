...
1. A backend job provides new hires with a uniquely identifiable link to the employee onboarding portal from (A) that represents the new hire’s specific process. For this use case, the account for the new hire should already be provisioned in Microsoft Entra ID. Consider using [Lifecycle Workflows](~/id-governance/what-are-lifecycle-workflows.md) as the triggering point of this flow.

1. New hires select the link to the portal in (A) and are guided through a wizard-like experience:
    ...
    1. New hires present the Verified ID acquired in step 1.
    1. System receives the claims from the identity verification partner, performs the new hire user account lookup, and validates the information.
    1. System executes the onboarding logic to locate the Microsoft Entra account of the user, and [generate a temporary access pass using MS Graph](/graph/api/resources/temporaryaccesspassauthenticationmethod?view=graph-rest-1.0&preserve-view=true).
