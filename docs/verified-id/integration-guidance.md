...
1. Upon successful completion, IDV kicks off Verified ID issuance flow and issues a Verified ID. At this point, user is presented with a deep link or QR code to **Add Card** in Microsoft Authenticator application. The IDV website receives a successful issuance callback from Verified ID service.
    > [!NOTE]
...
1. IDV redirects the user back to the customer’s relying party application. 

1. For the remainder of the steps in the diagram (that is, from steps 14 to 16), user is asked to present Verified ID with FaceCheck. On successful presentation, user is onboarded to the system.
...
1. Set up Microsoft Entra Verified ID Service: using [Quick setup](verifiable-credentials-configure-tenant-quick.md) or [Advanced setup instructions](verifiable-credentials-configure-tenant.md).
1. If you have details like **VCType** and **did** from the IDV partner, then you can use the payload reference from [Presentation Request API](presentation-request-api.md) section to verify Verified IDs issued by Identity verification partners (IDVs). 
1. Customers can also generate the presentation request API payload using the following steps:
    1. Go to **Microsoft Entra admin center** -> **Verified ID**.
    1. Select **Create Verification Request** tab.
    ...
    1. Choose **Select first user**.
    1. Look for the respective **IDV** in the Search/select issuers drop-down menu by typing their name for example *woodgrove.com*.
    1. Select the credential type that your application requires from the IDV for verification. This is also referred to as *VCType* in Presentation Request API payload.
    1. Select **Add** and then select **Review**.
    1. Download the request body and copy/paste POST API request URL.
    1. You now have the request URL and body from your tenant admin and can follow these steps to update your application or website. To request Verified IDs from your users, include the request URL and body in your application or website.
        > [!NOTE]
...
## Next steps
Learn more about how to integrate partner solutions with your application:
