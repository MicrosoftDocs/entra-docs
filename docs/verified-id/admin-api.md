...
Use this endpoint to create or update a Verifiable Credential service instance.
...
The following sections describe the `didModel` properties for each trust system type.
...
This call creates a new **private key** and stores the key in the specified Azure Key Vault, sets the permissions to this Key Vault for the verifiable credential service, and creates a new **DID** with a corresponding DID Document.
...
> [!NOTE]
> You can create multiple authorities with their own DID and private keys, but they aren't visible in the UI of the Azure portal. Currently only one authority is supported. Not all scenarios have been fully tested with multiple created authorities.
...
The `generateWellknownDidConfiguration` method generates the signed `did-configuration.json` file. The file must be uploaded to the `.well-known` folder in the root of the website hosted for the domain in the linked domain of this verifiable credential instance. For instructions, see [Verify domain ownership](how-to-dnsbind.md#verify-domain-ownership-and-distribute-the-did-configurationjson-file).
...
This endpoint allows you to create new contracts and update existing contracts. Contracts consist of two parts represented by two JSON definitions. For more information on how to design and create a contract, see [Credential design](credential-design.md).
...
| `allowOverrideValidityIntervalOnIssuance` | boolean | If the createIssuanceRequest API call is allowed to override expiry of the credential. This is only valid for [idTokenHint](#idtokenhintattestation-type) flows. |
...
When you create a contract, the name you use has to be unique in the tenant. If you create multiple authorities, the contract name has to be unique across all authorities.
...
[Search](how-to-issuer-revoke.md) for verifiable credentials with specific index claims. Since only a hash is stored, you need to search for the specific calculated value. The algorithm you need to use is: Base64Encode(SHA256(contractid + claim value)) An example in C# looks like this:
...
> [!NOTE]
> If you don't have delete permissions on Key Vault, an error message is returned and the opt-out still proceeds.
