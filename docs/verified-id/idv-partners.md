...
- **Verified ID API-based integration**:  Microsoft Entra Verified ID offers REST APIs that customers and partners can use for identity verification and proofing services integration from IDV partners. This pattern requires customer to set up billing contracts directly with the IDV partner and the use of REST APIs for integration. The integration guidance page covers step by step process and flows for issuer and verifier integrations.
