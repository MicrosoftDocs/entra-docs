...
1. Go over the Microsoft Entra Verified ID [deployment steps](verifiable-credentials-configure-tenant-quick.md) to reconfigure the service.
    1. If you are manually setting up Verified ID, it's recommended that you choose a location for your Azure Key Vault, to be in the same or closest region to avoid performance and latency issues. 
...
If you want to move from `did:web` to `did:ion`, you can follow these steps via the [Admin API](admin-api.md) (Please note that changing authority requires reissuance of all credentials):
...
After you have created your new `did:web` authority, you need to recreate your credential definitions. You can either do that via the [portal](https://entra.microsoft.com/#view/Microsoft_AAD_DecentralizedIdentity/CardsListBlade) if you opted-out and re-onboarded, or you need to use the [create contract](admin-api.md#create-contract) to recreate them.
...
If you didn't opt-out and re-onboarded, you need to remove your old `did:ion` authority.
