...
Most of authorities are already FIPS compliant. It's only authorities created before February 2024 using [Advanced Setup](verifiable-credentials-configure-tenant.md) method that require signing key upgrade. `Quick Setup` authorities aren't affected as they already use the compliant P-256 signing key.
...
Any signing activities, like issuing credentials or making presentation requests, are be signed by your new P-256 key. 
...
Eventually, you have no credentials signed by the old key exists as they all have expired. If your credentials have a long lifetime before they expire, and you want to stop using the old P-256K, you should consider instructing your users to reissue in advance.
