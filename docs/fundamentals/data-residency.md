...
#Customer Intent: As an IT admin, I want to understand Microsoft Entra ID data residency so that I can ensure compliance with data location requirements.
---
...
|Microsoft Entra Backup and Recovery snapshots|Microsoft automatically takes snapshots of certain directory data at regular intervals, which are stored in Microsoft-managed Azure storage and replicated to multiple datacenters in the same region as the tenant. Replication creates high availability and multiple reading and writing endpoints. Snapshots are encrypted using the AES-256 cryptography algorithm, using Microsoft-managed keys. No Microsoft Entra permissions or roles are available to delete or modify snapshots created by Microsoft Entra Backup and Recovery.|In geo location|
