...
Identity admins using [Security Copilot as a part of the Microsoft Entra admin center](./copilot-entra-security-scenarios.md#scenarios-supported-by-microsoft-security-copilot-in-microsoft-entra choose from a set of app risk starter prompts that appear at the top of the Security Copilot window. Select from suggested prompts that may appear after a response. App risk starter prompts appear in application-related admin center blades: **Enterprise apps**, **App Registrations**, and **Identity Protection Risky workload identities**.
