...
## Multiple product licenses and group assignments
...
One potential work-around is to use [dynamic membership groups](~/identity/users/groups-dynamic-rule-member-of.md). You could:
1. Create dynamic groups to assign primary licenses (like Exchange Online)
2. Use a second dynamic group with the same membership rules to apply prerequisite licenses
3. Assign other licenses after the initial group applies the primary license
...
We recommend that you test the new membership rule beforehand to make sure that the new membership in the group is as expected. If you encounter errors during your test, see [Use audit logs to monitor group-based licensing activity](/azure/security/fundamentals/log-audit).

## Microsoft Entra ID Mail and ProxyAddresses attribute changes
