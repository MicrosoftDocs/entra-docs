...
- One of the following roles: **Global Secure Access Administrator** or **Security Reader**.
...
| Field | Description |
|---|---|
| **Creation time** | Timestamp when the event was recorded. |
| **Destination URL** | The URL of the remote MCP server. This value is the unique identifier for the MCP server. |
| **User** | The user or identity whose traffic was inspected. |
| **Activity** | The traffic type, which is **MCP** for MCP traffic events. |
| **Sub-activity** | The specific MCP API call, such as `initialize`, `tools/list`, `tools/call`, or `prompts/list`. These correspond to the standard MCP protocol operations. |
| **Response side** | Indicates whether the entry is a **request**, **response**, or **event**. |
