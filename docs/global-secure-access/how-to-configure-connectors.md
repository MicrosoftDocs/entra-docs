...
> [!WARNING]
> If you've deployed Microsoft Entra Password Protection Proxy, don't install Microsoft Entra application proxy and Microsoft Entra Password Protection Proxy together on the same machine. Microsoft Entra application proxy and Microsoft Entra Password Protection Proxy install different versions of the Microsoft Entra Connect Agent Updater service. These different versions are incompatible when installed together.
...
To prepare your environment for Microsoft Entra application proxy, start by enabling communication to Azure data centers. If there's a firewall in the path, make sure it's open. An open firewall allows the connector to make HTTPS (TCP) requests to the Application Proxy.
...
**To install the connector from the Microsoft Entra admin center**:
...
1. Follow the instructions in the wizard to install the service. When you're prompted to register the connector with the Application Proxy for your Microsoft Entra tenant, provide your Application Administrator credentials.
    * For Internet Explorer (IE): If IE Enhanced Security Configuration is set to On, you might not see the registration screen. To get access, follow the instructions in the error message. Make sure that Internet Explorer Enhanced Security Configuration is set to Off.
...
## Minimize impact during connector server maintenance
...
1. Create a maintenance connector group
   - In the Microsoft Entra admin center, create a new connector group that is assigned **only** to a Private Access test application.
   - This group acts as a “parking spot” for maintenance.
1. Move the connector into the maintenance connector group
   - When you're ready to service a connector server, edit its assignment and move it into the maintenance connector group with a test Private Access app where there's no active traffic.
   - After the connector is moved to the maintenance group, it no longer receives new user connections. Existing connections continue until they finish gracefully.
...
1. Perform maintenance
   - Apply patches and updates, and reboot the server as needed.
   - Ensure that the Microsoft Entra private network connector services restart and are healthy before returning the connector to production.
   - After patching, send requests to the test application to validate end-to-end connectivity.

1. Return the connector to its original group
   - After maintenance is complete and services are running, move the connector back into its original connector group.
   - The connector again receives traffic as part of that group’s high-availability pool.
