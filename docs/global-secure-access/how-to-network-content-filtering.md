...
1. On the **Rules** tab:  
    ...
    1. (Optional) Configure the **Source type** condition (preview) to scope the rule by traffic origin. Select **Agent** to match traffic from AI agents, such as Microsoft Copilot Studio agents and autonomous AI tools, or **User** to match traffic from signed-in users, browsers, and applications. When a request doesn't originate from an AI agent, traffic is classified as **User**. If you don't configure a source type, the rule applies to all traffic.
