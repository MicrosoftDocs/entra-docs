...
- Network calls to Dataverse and Azure SQL knowledge sources aren't supported. 
- Network calls to the following custom tools aren't supported: Prompt, Agent flow, Computer Use, and Child Agents.
- Network calls to Large Language Model (LLM), either for orchestration or results enhancement, aren't supported.
