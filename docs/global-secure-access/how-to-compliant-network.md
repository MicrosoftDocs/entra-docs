...
1. After disabling the Global Secure Access client, access another application integrated with Microsoft Entra ID single sign-on. For example, you can try signing in to the Azure portal. Your access should be blocked by Entra ID Conditional Access.

> [!NOTE]
> If you are already signed in to an application, access will not be interrupted. Compliant Network condition is evaluated by Microsoft Entra ID, and if you are already signed in, you may have an existing session with the application. In this scenario, Compliant Network check will be re-evaluated next time the Microsoft Entra ID sign-in is required, when your application session has expired.
> 
