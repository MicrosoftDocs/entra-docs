...
- **Enabled the Internet Access traffic forwarding profile**: This profile activated the GSA client's ability to tunnel internet-bound traffic to the Microsoft SSE.
- **Understood traffic flow**: Internet traffic now flows from the user device, to the GSA client, to the Microsoft SSE proxy, and finally to the internet destination.
