...
> [!IMPORTANT]
> Operating Private Access isn't a dashboard-watching exercise. Before you add a recurring task to an operator's calendar, check whether it's already covered by the [Zero Trust Assessment](#automated-posture-assessment), a built-in Sentinel analytics rule, or a scheduled [playbook](#automation-playbooks).
...
| --- | --- | --- |
| **Quick Access** | Single enterprise application containing multiple application segments (fully qualified domain names (FQDNs), IPs, IP ranges) used to enable VPN replacement. Shared Conditional Access policy across all segments. | Simpler to manage, but a policy change affects all associated application segments. Monitor as a single unit. |
...
To use full ZTNA application segmentation, migrate from Quick Access to Per-app enterprise applications. The Zero Trust Assessment helps you track these efforts.
...
| Configuration backup compliance | Network Ops L2 | [Playbook 3](#playbook-3-weekly-configuration-backup) + [`Test-GsaBackupCompliance.ps1`](scripts/powershell-test-backup-compliance.md) | Backup compliance script runs after Playbook 3 and alerts if files are missing or stale. | Troubleshoot the runbook or script. Manually export via Graph API as a fallback. |
| Alert noise ratio | SOC | [`Test-GsaAlertNoiseRatio.ps1`](scripts/powershell-test-alert-noise-ratio.md) scheduled weekly | Script reports analytic-rule-to-incident ratio per Private Access rule. | Tune high-noise rules. Close the loop with Sentinel tuning recommendations. |
...
| Role-based access control (RBAC) review | IAM Admin | ZT Assessment *Application admin rights are constrained* + [`Test-GsaRbacHygiene.ps1`](scripts/powershell-test-role-based-access-control-hygiene.md) | Assessment checks role-scope hygiene; script reports standing assignments, stale admins, and admins without phishing-resistant MFA. | Remove access for accounts that no longer require it. Enforce phishing-resistant MFA for all admins. |
...
Integration and automation are the core operational value of a cloud-delivered ZTNA service. Automate routine tasks to reduce human error and free your team for higher-value work. Several playbooks in this section open IT service management (ITSM) tickets—adapt the connector references to match your platform.
...
> [!IMPORTANT]
> APIs under the `/beta` version in Microsoft Graph are subject to change. Use of these APIs in production applications isn't supported. To determine whether an API is available in v1.0, use the **Version** selector.
...
7. Save, enable, and test by disabling a connector in a nonproduction connector group.
...
6. Test in a nonproduction environment by disabling all connectors in a test group.
...
6. Schedule [`Test-GsaBackupCompliance.ps1`](scripts/powershell-test-backup-compliance.md) to run the day after each backup. The script verifies that expected files landed in the container, are nonempty, and are newer than the previous run—and raises an alert if not. This watchdog closes the loop so missing backups surface as an incident, not during the next weekly check.
...
6. Add a [Grafana](/azure/managed-grafana/) or Sentinel-workbook tile showing "Zero Trust Assessment pass rate (Private Access)"—this tile becomes the single posture number for management reporting.
...
5. This analytics rule replaces the previous *daily manual audit-log review*—operators don't need to run the KQL query manually anymore.
...
2. Run the [application access failures query](#kql-queries-for-private-access-monitoring) against a seven-day window.
...
1. **Alert-to-ticket**: Use [Playbook 2](#playbook-2-auto-create-itsm-ticket-for-connector-group-failure) as a starting point. Extend the same pattern with the [ServiceNow connector for Logic Apps](/connectors/service-now/) to cover more Private Access alerts.
