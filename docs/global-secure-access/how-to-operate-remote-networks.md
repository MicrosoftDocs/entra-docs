...
Remote Networks operations should start with alerts that identify branch connectivity failures before users report them. Use this section to define the critical tunnel and site-level alerts, deploy the supporting KQL queries for ongoing monitoring, and establish the daily and weekly checks that help you track tunnel stability, capacity, and failover readiness across each remote site.
...
| BGP session down (if applicable) | BGP peering session with GSA drops | Network Security Engineer | 1. Check BGP configuration on the CPE device. 2. Verify the peer IP and ASN settings. 3. Review route advertisements for changes. |
| Tunnel MTU-related packet drops | High rate of fragmented or dropped packets on the tunnel | Network Security Engineer | 1. Reduce the tunnel MTU on the CPE device (recommended: 1400 bytes for GRE, 1380 for IPsec). 2. Enable TCP MSS clamping if not already configured. |
...
| IKE/IPsec security association review | Network Security Engineer | Verify that SA lifetimes and ciphers align with your security policy. | Update tunnel parameters to meet current security standards. |
...
4. Wait 2–5 minutes. Verify that traffic has shifted to the secondary tunnel.
...
| Active tunnels per site | 2+ (minimum for redundancy) | Only 1 active tunnel | No active tunnels |
...
| 6 | **Tunnel flap report** | Azure Automation runbook: queries tunnel status transitions over the past 7 days, generates a summary report | Scheduled: weekly |

### Network management tool integration

If your organization uses a network management platform (SolarWinds, PRTG, ThousandEyes, or similar):

1. **Tunnel health monitoring**: Configure your network tool to monitor the GRE/IPsec tunnel endpoints as monitored interfaces. This gives you a second monitoring layer alongside Sentinel.
...
3. **End-to-end path monitoring**: Use synthetic transactions or ping monitors from branch sites through the GSA tunnels to verify end-to-end connectivity and measure latency.
