...
description: "Post-deployment operations guide for Microsoft Entra Internet Access, the secure web gateway (SWG) capability, covering alerting, health checks, web filtering policy management, integration, and automation."
ms.topic: how-to
ms.date: 05/04/2026
...
This guide covers day-to-day operations for Microsoft Entra Internet Access after deployment. The procedures cover alerting, health checks, web filtering policy management, Transport Layer Security (TLS) inspection, and automation. They apply only to Internet Access traffic profiles.
...
| Configuration backup compliance | Network Ops L2 | [Playbook 3](#playbook-3-weekly-web-filtering-policy-backup) + `Test-GsaBackupCompliance.ps1` | Backup compliance script runs after Playbook 3 and alerts if files are missing or stale. | Troubleshoot the runbook or script. Manually export via Graph API as a fallback. |
...
| Alert noise ratio | SOC | `Test-GsaAlertNoiseRatio.ps1` scheduled weekly | Script reports analytic-rule-to-incident ratio per Internet Access rule. | Tune high-noise rules. Close the loop with Sentinel tuning recommendations. |
| Prompt injection policy validation | IAM Admin | *Manual* | Run the [prompt injection validation procedure](#validate-prompt-injection-protection) from a test device. Confirm TLS inspection, policy enforcement, and AI Insights log ingestion for each target AI site. | Reverify TLS inspection and QUIC status. Check prompt policy rule configuration and conversation scheme endpoints. See [Validate prompt injection protection](#validate-prompt-injection-protection). |
...
| Role-based access control (RBAC) review | IAM Admin | `Test-GsaRbacHygiene.ps1` | Script reports standing assignments, stale admins, and admins without phishing-resistant MFA. | Remove stale access. Verify MFA enforcement for all admin accounts. |
...
Validate each prerequisite before testing prompt injection enforcement. If any prerequisite fails, prompt injection policies can't inspect traffic.
...
1. **Identify the test scope.** Select one or more AI sites to validate against (for example, ChatGPT, Claude, Gemini). Confirm each site is included in the prompt policy rules under **Global Secure Access** > **Secure** > **Prompt policies**.
...
> [!TIP]
> If AI Insights logs don't appear after 5 minutes, revalidate TLS inspection (prerequisite 1), browser configuration (prerequisite 3), and policy attachment (step 2). Use the [TLS inspection status KQL query](#kql-queries-for-internet-access-monitoring) to confirm the target AI site traffic is being inspected.
...
2. Run the [configuration backup playbook](#playbook-3-weekly-web-filtering-policy-backup) to capture a prechange snapshot.
...
Global Secure Access doesn't acquire QUIC traffic. If QUIC is enabled on a device, TLS inspection doesn't work for sites that negotiate QUIC—this group includes ChatGPT, Claude, Gemini, and most AI sites.
...
Internet Access operations should rely on integrated monitoring and scheduled automation rather than manual portal checks. Use the procedures in this section to:

- Export configuration for backup and change tracking.
- Connect Internet Access telemetry to Microsoft Sentinel for alerting and incident routing.
- Implement playbooks for common operational tasks such as threat notifications, exception-ticket creation, and recurring policy backups.
...
4. **Configure automation rules**: For each enabled rule, go to **Automated response** > **Add new**. Create an automation rule that assigns Internet Access incidents to your operations team and optionally triggers a Logic Apps playbook for notification or IT service management (ITSM) ticket creation.
...
> [!TIP]
> If you already completed Sentinel integration for Private Access, you can skip steps 1 and 2—the diagnostic settings and content pack are shared across all Global Secure Access capabilities. Start at step 3 to enable the Internet Access-specific rules.
...
2. Open the enabled rule. Then select **Automated response** > **Add new**.
...
5. Add a **Post message in a channel** Teams action. Include: user principal name, destination fully qualified domain name (FQDN), URL category (malware/phishing/C2), timestamp, and a direct link to the Sentinel incident.
...
#### Playbook 2: Autocreate ITSM ticket for policy override requests
...
1. Validate the change request is approved and includes: FQDNs to add or remove, business justification, and expected review date.
2. Run the [configuration backup playbook](#playbook-3-weekly-web-filtering-policy-backup) to capture a prechange snapshot.
...
Use these steps to create a new TLS inspection policy or modify an existing one. For bypass list changes, use the preceding script.
...
2. Validate that the change request is approved and that the policy owner reviewed the CSV.
3. Run the [configuration backup playbook](#playbook-3-weekly-web-filtering-policy-backup) to capture a prechange snapshot.
...
6. Add a [Grafana](/azure/managed-grafana/) or Sentinel-workbook tile showing "Zero Trust Assessment pass rate (Internet Access)"—this tile becomes the single posture number for management reporting.
...
| **Required permissions** | Microsoft Sentinel Contributor; Logic Apps access to your IT service management (ITSM) or configuration management database (CMDB) API |
...
5. This analytics rule replaces the previous *daily manual audit-log review*—operators don't need to query `AuditLogs` manually anymore.
...
2. Run the [blocked-traffic-by-category query](#kql-queries-for-internet-access-monitoring) against a seven-day window.
...
5. Email to the Network Ops Lead and Capacity Planner with recommended actions (for example, *"Engage network team—sustained > 80% of baseline for five consecutive days"*).
...
1. **Alert-to-ticket**: Use [Playbook 2](#playbook-2-autocreate-itsm-ticket-for-policy-override-requests) as a starting point. Extend the same pattern with the [ServiceNow connector for Logic Apps](/connectors/service-now/) to cover more Internet Access alerts.
...
| Client agent compliance | Network Ops L2 | % of managed devices with current Global Secure Access client version | > 98% | Weekly |
