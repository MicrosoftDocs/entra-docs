...
description: "Learn how to troubleshoot Transport Layer Security (TLS) inspection errors in Global Secure Access."
...
#customer intent: I want to troubleshoot Global Secure Access TLS inspection issues.
...
Alternatively, you can append the certificate to Git's Certificate Authority bundle:
...
Python-based tools, such as Azure CLI, will fail without the correct certificate authority. You can install pip-system-certs so Python trusts the Windows certificate store:
...
Alternatively, you can append the certificate directly to Python's certifi CA bundle:
...
For early preview customers with legacy a TLS configuration, when creating a Certificate Signing Request (CSR), you may see this error: `Cannot create external certificate, an internal certificate already exists for tenant.`
