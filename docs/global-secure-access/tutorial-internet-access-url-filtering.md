...
URL filtering is an advanced type of web content filtering based on a full or partial URL. Unlike filtering based on FQDNs which are visible in the traffic header, URL filtering requires TLS inspection to see the specific destination a user is attempting to access. For example, `www.bing.com` is visible without TLS inspection, but `www.bing.com/images` isn't. URL filtering provides fine-grained web content filtering and enhances the accuracy of web content filtering by category.
...
- Complete [Tutorial: Configure TLS inspection](tutorial-internet-access-tls-inspection.md). TLS inspection is a prerequisite for URL filtering.
...
This tutorial builds on the previous tutorials. In the FQDN filtering tutorial, you blocked access to Bing and received an error that isn't user-friendly. In the TLS inspection tutorial, you enabled TLS inspection, which is a prerequisite for URL filtering. In this tutorial, you will:
...
The following video demonstrates the end user experience of URL filtering:
...
1. Select **Add rule**.
   ...
   - **Destination:** `www.youtube.com/shorts,youtube.com/shorts`
1. Select **Add**.
1. Select **Next**, then select **Create policy**.

### Create a policy to allow Bing Maps

1. Select **Create policy**.
   - **Name:** Allow Bing Maps
   - **Action:** Allow
1. Select **Next**.
1. Select **Add rule**.
   - **Name:** Bing Maps
   - **Destination type:** url
   - **Destination:** `*.bing.com/maps,bing.com/maps`
1. Select **Add**.
1. Select **Next**, then select **Create policy**.

## Step 2: Link web content filtering policies to a security profile

1. Browse to the **Security Profiles** blade.
1. Select the security profile from the TLS inspection tutorial (not the baseline security profile), then select the **Link policies** blade.
1. Select **Link a policy**, then select **Existing web filtering policy**.
1. Under **Policy name**, select "Block YouTube Shorts" and give it a priority of **200**. State should be **Enabled**. Select **Add**.
1. Select **Link a policy**, then select **Existing web filtering policy** again.
1. Under **Policy name**, select "Allow Bing Maps" and give it a priority of **150**. State should be **Enabled**. Select **Add**.
1. Select **Next**.
...
> [!NOTE]
> Newly created security profiles can take up to an hour to take effect. If you linked the new rules to an existing security profile that's already assigned to your user via Conditional Access, it should take effect in a few minutes.

1. On your test device, open a browser and navigate to `www.bing.com`. Verify you're blocked and that the custom error message is displayed.

   ![Custom error page showing the custom block message for a blocked site.](media/tutorial-internet-access-url-filtering/url-filtering-custom-block-page.png)

1. Navigate to `www.bing.com`. Verify access is still blocked from the FQDN filtering tutorial.
1. Navigate to `www.bing.com/maps`. Verify access is allowed.
1. Navigate to `www.youtube.com`. Verify access is allowed.
1. Navigate to `www.youtube.com/shorts`. Verify access is blocked.

**Policy evaluation order**

In this tutorial's example, the policy evaluation works as follows:
...
1. **Implemented fine-grained URL filtering** - You blocked YouTube Shorts while allowing the rest of YouTube, demonstrating how URL filtering enables precise control over web destinations.

1. **Created exception rules** - You allowed Bing Maps while Bing itself remains blocked by the baseline profile. This shows how to layer policies for nuanced access control.

1. **Configured custom block messages** - Users now see a helpful error page instead of a generic "connection reset," improving user experience and reducing helpdesk tickets.

1. **Understood policy precedence** - Lower priority numbers are evaluated first, allowing custom profiles to create exceptions to baseline rules.
