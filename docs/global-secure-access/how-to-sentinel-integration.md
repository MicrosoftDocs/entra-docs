...
The following analytics rules are included in the Global Secure Access solution:
...
The Global Secure Access solution includes three workbooks:

- **Enhanced Microsoft 365 logs** — Combines Microsoft 365 `OfficeActivity` data with Global Secure Access `NetworkAccessTraffic` logs to provide enriched audit event context, including device ID, operating system, and original IP address.
- **Network Traffic Insights** — Visualizes network traffic patterns, policy actions, and usage trends to help you identify anomalies and optimize security policies.
- **Global Secure Access Alerts** — Surfaces security alerts generated by Global Secure Access, including unhealthy remote networks and token misuse, with visual dashboards for investigation and triage.
