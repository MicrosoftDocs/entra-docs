...
1. In the **Logs** section, select the following **Categories**:
   ...
   |---|---|
   | [NetworkAccessTrafficLogs](how-to-view-traffic-logs.md) | Captures all network traffic routed through Global Secure Access, including source/destination IPs, ports, protocols, and policy actions (allow/block). Use these logs for traffic analysis, policy validation, and threat investigation. |
...
|---|---|
| **Enriched Microsoft 365 logs** | Correlates Microsoft 365 `OfficeActivity` data with Global Secure Access `NetworkAccessTraffic` logs via Unique Token ID to enrich SharePoint Online events with device ID, operating system, and original source IP, improving visibility into user activities and potential security threats. |
| **Network Traffic Insights** | Interactive dashboard that visualizes traffic patterns, top destinations, user activity, blocked traffic, bandwidth trends, and traffic type breakdown (Internet, Microsoft 365, Private Access) to help you identify anomalies and optimize security policies. |
| **MCP Servers Dashboard** | Visualizes Model Context Protocol (MCP) traffic, including AI agent activity, MCP client/server breakdown, sub-activity patterns, and GenAI app usage across the organization. |
