...
- Create [Connector Group](/graph/api/connectorgroup-post?view=graph-rest-beta&tabs=http) using Graph API. Use 'region' property to assign the region to 'japan'.  E.g., Run the POST request below with region set to "japan" to create a connector group assigned to Japan.
