...
To prevent service disruptions and ensure secure, reliable access, use a layered defense strategy. Microsoft strengthens identity protection by integrating with leading non-Microsoft WAF and bot mitigation providers. The integrations enable real-time analysis and filtering high-risk or malicious traffic, before it reaches your identity endpoints.
...
|---|---|
|Rate limiting and adoptive throttling|Rate limiting is an effective and configurable defense against volumetric and low-and-slow attacks. Leading WAFs support fine-grained policies that limit requests per second, or per minute at various levels: per IP, endpoint, or session. Adaptive throttling automatically tightens restrictions in response to sudden traffic spikes, known attack patterns, or anomalies in behavior.</br> For example, configure a sign-in endpoint for a set number of authentication attempts within a short timeframe. This effort reduces the risk of credential stuffing, and brute force attacks. Also, dynamically adjust rate limits based on contextual signals such as geolocation, request headers, or device fingerprinting.|
...
|Bot protection|We recommend integrated bot protection powered by machine learning. These capabilities help detect and prevent automated attacks using a range of enforcement options, from silent blocking to interactive challenges such as CAPTCHA or re-CAPTCHA.|
...
|---|---|
|Prefer certificates over secrets|When possible, use X.509 certificate credentials instead of password secrets. While secrets are easier to set up, certificates offer stronger security and can be the default method to acquire tokens.</br> Currently, managed identities and workload identities aren't available in Microsoft Entra External ID.|
...
In this section, learn how to protect users from sign-up fraud. The user flow design helps determine the likelihood of sign-up fraud. Sign-up journeys that perform minimal user verification are susceptible to fraudulent activity. Controls, such as advanced fraud protection, distinguish human users from bots. Also, you can deter automated abuse. Microsoft Entra External ID supports various controls that introduce degrees of user friction. The following table has a list of security controls for user security
...
|Security control|Details|
|---|---|
|Microsoft Entra Conditional Access policy: Step-up MFA with authentication context|Apps can dynamically enforce [Microsoft Entra multifactor authentication (MFA)](../external-id/customers/concept-multifactor-authentication-customers.md) by using authentication context in [Conditional Access](../identity/conditional-access/overview.md) policies. Use [step-up MFA](../external-id/customers/concept-multifactor-authentication-customers.md#step-up-mfa-with-conditional-access-authentication-context) for high-value activities like financial transactions, access to sensitive data, or privileged operations. MFA isn't required for the entire app — it's prompted only when the app requests the authentication context. This approach supports Zero Trust principles by enforcing least-privilege access while keeping lower-risk interactions frictionless.</br></br>For example, a user browses an ecommerce site and adds items to their cart without an MFA prompt. At checkout, the app requests the authentication context, and the Conditional Access policy prompts the user for MFA before the payment completes. App developers add the request from inside the app using [Conditional Access authentication context](../identity-platform/developer-guide-conditional-access-authentication-context.md).|
...
In large systems, some transactions fail. Incomplete identity experiences, sometimes called incomplete conversions, can occur due to distracted users, telco failures, and browser crashes. On a large scale, addressing every failure is impractical. Set up a baseline for typical failure events. Also set up monitoring and alerting to detect deviations, like performance monitoring. Use the following job aid to record failure metrics.
