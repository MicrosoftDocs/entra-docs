...
Users often encounter access denied errors in Entra Copilot when attempting actions that require elevated privileges, such as viewing sign-in logs or listing admin role assignments. Identifying the correct role and ensuring it’s the least-privileged option can be complex and time-consuming. This combined with finding the right avenue to get that assignment can be overwhelming.  

By using best practices from our least privileged access framework, Entra Copilot intelligently determines the least privileged role based on the desired task, checks for the eligible assignments, and enables the user through Just-in-Time (JIT) activation using Entra Privileged Identity Management (PIM). This provides a seamless experience within Entra Copilot itself and reduces the burden for users who had to leave their Entra Copilot chat, navigate to the Entra portal, activate the role manually, and return to retry the query. This feature keeps everything in one continuous conversation, resulting in reduced friction, improved productivity, and secure access management.
...
- [Assess application risks using Microsoft Security Copilot in Microsoft Entra](./entra-id-investigate-risky-apps.md)
