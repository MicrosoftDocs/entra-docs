...
Customers with Permissions Management Administrator role have first to onboard Permissions Management on their Microsoft Entra tenant, and then onboard their AWS accounts, GCP projects, and Azure subscriptions. More details about onboarding can be found in our product documentation.
...
Customers can also remove, export or modify specific data if a Permissions Management Administrator or Global Administrator using the Permissions Management service files an official Data Subject Request. To file a request:
