...
|`oob`| No |The one-time passcode that the user received in their email. The value for this parameter is the one-time passcode that the user receives in their email address. This parameter is **required** if the authentication method is email one-time passcode or the app is submitting the email one-time passcode to complete an MFA challenge. To **resend a one-time passcode**, the app needs to make a request to the `/challenge` endpoint again. |
