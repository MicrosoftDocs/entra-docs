...
| `mfa_required` | MFA is required. The response includes a [continuation token](../../reference-native-authentication-api.md#continuation-token). Call the `oauth2/v2.0/introspect` endpoint to get the user's registered strong authentication methods. **This suberror only occurs when the user's primary authentication method is email with password.** See how to [get a user's registered strong authentication methods](../../reference-native-authentication-api.md#get-user-registered-strong-authentication-methods). <br><br>**Note**: In some cases MFA is required but native authentication doesn't return `mfa_required`. or example, if a [strong authentication method registration](../../reference-native-authentication-api.md#register-a-strong-authentication-method-api-reference) flow precedes a call to the `/oauth2/v2.0/token` and the only available method (email) was already verified during that flow.|
