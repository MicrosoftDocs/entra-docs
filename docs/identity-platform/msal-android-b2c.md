...
The B2C identity management system supports authentication with a number of social account providers such as Google, Facebook, X, and Amazon. If you plan to support such account types in your app, it is recommended that you configure your MSAL public client application to use either the `DEFAULT` or `BROWSER` value when specifying your manifest's [`authorization_user_agent`](msal-configuration.md#authorization_user_agent) due to restrictions prohibiting use of WebView-based authentication with some external identity providers. Additionally, WebView-based authentication also does not currently support handling custom URI schemes including otpauth://. If you plan to host and redirect to such URIs in your application's authentication flow, then it is further recommended to use DEFAULT or BROWSER as authorization_user_agent.
