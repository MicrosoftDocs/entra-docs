...
The minimum set of URLs that need to be allowed for the SSO plug-in to function on Platform SSO targeted devices or on operating system versions released before 2022:
  ...
  > [!NOTE]
  > Platform SSO is incompatible with the Microsoft Entra ID Tenant Restriction v2 feature when Tenant Restrictions are deployed using a corporate proxy. Alternative option is listed in [Known limitation](/entra/external-id/tenant-restrictions-v2#known-limitation)
