...
Many modern apps have a single-page app (SPA) front end written primarily in JavaScript, often with a framework like Angular, React, or Vue. The Microsoft identity platform supports these apps by using the [OpenID Connect](v2-protocols-oidc.md) protocol for authentication and one of two types of authorization grants defined by OAuth 2.0. We recommend that you [use the authorization code flow with PKCE](https://devblogs.microsoft.com/identity/migrate-to-auth-code-flow/) as it offers better security.
