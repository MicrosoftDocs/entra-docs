...
These claims are restricted by default, but aren't restricted if you have a [custom signing key](jwt-claims-customization#configure-a-custom-signing-key). Avoid setting `acceptMappedClaims` in the app manifest.
...
These claims are restricted by default, but aren't restricted if you have a [custom signing key](jwt-claims-customization#configure-a-custom-signing-key):
