...
*Authorization* is the act of granting an authenticated party permission to do something. It specifies what data you're allowed to access and what you can do with that data. Authorization is sometimes shortened to *AuthZ*. The Microsoft identity platform uses the [OAuth 2.0](https://oauth.net/2/) protocol for handling authorization, but there are various other authorization systems at Microsoft, such as [Entra built-in roles](./identity/role-based-access-control/permissions-reference.md) and [Exchange RBAC](~/exchange/permissions-exo/application-rbac.md).
