...
1. To enable multifactor authentication (MFA) for your customers, follow the steps in [Add multifactor authentication (MFA) to an app](/entra/external-id/customers/how-to-multifactor-authentication-customers). Currently, native authentication supports email one-time passcode as a second factor for MFA, which is available only when the primary authentication method is email with password.
