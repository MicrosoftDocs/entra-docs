...
Apple Safari has an on-by-default privacy protection feature called [Intelligent Tracking Protection](https://webkit.org/tracking-prevention-policy/), or _ITP_. Chrome has a browser privacy intiative named the [Privacy Sandbox](https://developers.google.com/privacy-sandbox/overview). These initiatives encompass many different browser privacy efforts by the browsers and have different timelines. Both efforts block "third-party" cookies, cookies on requests that cross domains. Safari and Brave block third-party cookies by default, and Chrome announced that they'll start blocking third-party cookies by default [in the future](https://privacysandbox.com/open-web/#the-privacy-sandbox-timeline).The Privacy Sandbox includes changes to [partitioned storage](https://developers.google.com/privacy-sandbox/3pcd/storage-partitioning) as well as third-party cookie blocking. 
