...
> [!IMPORTANT]
>
>- You can now set up SAML/WS-Fed IdP federation with Microsoft Entra ID verified domains from other tenants. Then you can ensure users sign in with the federated IdP instead of Microsoft Entra ID by [configuring the invitation **Redemption order**](cross-tenant-access-overview.md#configurable-redemption) (in your cross-tenant access settings for inbound B2B collaboration). The verified domain must be in a separate tenant from where you set up federation.
