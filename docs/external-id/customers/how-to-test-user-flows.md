...
1. In the **Run user flow** pane, most of the fields are populated with values from the application registration, so you can leave the default values. (At least one application with a redirect URI must be associated with the user flow; see the [Prerequisites](#prerequisites)). Refer to the following table for details about each field.
   ...
   |**Access Tokens**     | If the application requires an access token to securely access a protected web API, these fields contain values from the application registration. The **Resource** is the resource (Web API) that accepts the token. **Scopes** defines the types of protected resources that the client application can access on behalf of the user or itself. [Learn more](../../identity-platform/access-tokens.md)       |
