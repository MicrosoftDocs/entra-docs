...
The following table lists the service limits we implement to prevent outages and slowdowns. These values apply to are lower during the preview phase, but we'll increase them as the product becomes generally available. [Learn more](~/identity/authentication/concept-mfa-telephony-fraud.md)
