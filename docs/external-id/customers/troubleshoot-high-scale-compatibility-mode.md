...
| Scenario | HTTP status | Error code | Self-serviceable |
|---|---|---|---|
| [Caller lacks required role](#caller-lacks-required-role) | 403 Unauthorized | `Unauthorized` | Yes |
| [Caller lacks Microsoft Graph permission](#caller-lacks-microsoft-graph-permission) | 403 Unauthorized | `Unauthorized` | Yes |
| [Wrong tenant type context](#wrong-tenant-type-context) | 400 BadRequest | `AccessDenied_NonB2CTenantNotAllowed` | Yes |
| [Parent tenant not enabled](#parent-tenant-not-enabled) | 400 BadRequest | `HybridUpgradeNotAllowed` | No |
| [DELETE on non-hybrid tenant](#delete-on-non-hybrid-tenant) | 403 Forbidden | `AccessDenied_NonHybridTenantNotAllowed` | Yes |
| [Stale GET response after POST](#stale-get-response-after-post) | 200 OK | N/A (cache) | Yes |
| [Custom attribute sync fails](#custom-attribute-sync-fails) | 400 BadRequest | `AADB2C99089` | Yes |
| [No subscription linked](#no-subscription-linked) | 400 BadRequest | `NoResourceProviderDataFound` | Yes |
...
Before calling POST to enable hybrid mode, patch every custom attribute that has a missing description:
...
1. For each attribute where `description` is null or empty, issue the PATCH request with a nonempty description.
