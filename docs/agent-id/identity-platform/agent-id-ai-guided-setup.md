...
Open the GitHub Copilot Chat panel and switch to **Agent mode**. Agent mode gives GitHub Copilot the ability to run terminal commands, read files, and interact with your environment — all capabilities required by the AI-guided setup.
...
1. **Validate prerequisites** — Confirms Frontier is enabled, checks Entra roles, validates that PowerShell 7+ and the Microsoft Graph beta module are installed.
...
6. **Create the blueprint principal** — Creates the service principal for the blueprint (this is **not** auto-created and must be done explicitly).
7. **Create agent identities** — Provisions one or more agent identity service principals under the blueprint.
...
- **Derived value confirmation** — Review auto-generated names and URIs before resources are created.
...
2. Navigate to **Agent ID** > **All agent identities** to see your new agent identity blueprint and any agent identities created under it.
...
|---|---|---|
| Prerequisites | Validates roles, Frontier access, PowerShell module, and Graph permissions | [Create a blueprint — Prerequisites](create-blueprint.md#prerequisites) |
...
The Agent ID APIs have several non-obvious requirements that the AI-guided setup detects and resolves automatically. Understanding these pitfalls is helpful if you need to debug issues or extend the setup.

### OData-Version header is required

All Agent ID API calls that use `@odata.type` require the `OData-Version: 4.0` header. Omitting this header results in the `@odata.type` field being silently ignored, causing the API to create a standard application instead of an agent identity blueprint. The AI-guided setup always includes this header.

### Blueprint principal is not auto-created
...
The AI-guided setup always creates the blueprint principal immediately after the blueprint. It also handles the idempotent case — if a previous run created the blueprint but crashed before creating the principal, the setup detects this and creates the missing principal.
...
### Agent identities cannot have password credentials
...
Using the standard `/applications/{id}/federatedIdentityCredentials` path may not work correctly for agent identity blueprints.
...
This happens when the `OData-Version: 4.0` header is missing or the `@odata.type` property is omitted. Verify both are present in the request. If using PowerShell cmdlets, ensure you're using the beta module and passing the correct body structure.
...
- Re-run the AI-guided setup with updated values — the idempotent checks will skip resources that already exist correctly.
