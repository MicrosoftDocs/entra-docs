...
| **User not compromised (False positive)** <br> ‘Risky users’ report shows at at-risk user but the user isn't compromised. | Select the user and then ‘Confirm user safe’. (This process confirms to Microsoft Entra ID that the user isn't compromised.) | Microsoft Entra ID moves the user risk to none [Risk state = Dismissed; Risk level = -]. | Clicking ‘Confirm user safe’ removes risk for the user and re-learns the usage properties. This action can't be undone. |
