...
| Failed access attempt | Offline  | Agent attempted and failed to access resources for which it is not authorized. This detection can indicate an attacker is attempting to replay an agent's token against an unauthorized resource. | failedAccessAttempt |
...
- **Confirm compromise**: After manual investigation or automated detection confirms the account is compromised, this is useful as part of incident response to prevent further damage. Confirm compromise automatically sets the risk level to High and creates an event in the agent's **Risk detections**. This action will trigger risk-based Conditional Access policies that are configured to block access on High Agent Risk. 
- **Confirm safe**: Marks the user as safe after investigation and clears any active risk state for that user by setting risk level to None. Use this when you want to mark a false positive and for the system to avoid flagging similar activity.
...
### Generating Synthetic Detections // May need to cut, Etan to confirm

<!--- Add links and clarify language when available. --->