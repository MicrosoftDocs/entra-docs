...
:::image type="content" source="~/media/external-identities/access-settings-users-sync.png" alt-text="Screenshot that shows the tab for cross-tenant synchronization tab with the checkboxes for synchronizing users and groups into a target tenant." lightbox="~/media/external-identities/access-settings-users-sync.png":::
