...
In the target tenant: Cross-tenant sync relies on the Azure AD External Identities billing model. To understand the external identities licensing model, see [MAU billing model for Azure AD External Identities](~/external-id/external-identities-pricing.md). You also need at least one Microsoft Entra ID P1 license in the target tenant to enable autoredemption.