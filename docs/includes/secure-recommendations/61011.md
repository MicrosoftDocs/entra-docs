...
Calls that don't reach Microsoft Entra can't be evaluated by Conditional Access policies, have no sign-in risk to calculate, and can't tie audit records to a specific user. The lack of this information makes it difficult to investigate and remediate. The runtime behavior that enforces Entra user authentication lives inside each agent's host (Copilot Studio, Microsoft 365 Copilot, Microsoft Foundry, custom code, third-party platforms) and is therefore not directly observable from the directory; what *is* observable is the trail left in the Microsoft Entra sign-in logs whenever an agent and its callers do go through Entra.
