...
2. Add one or more [custom domain names](~/fundamentals/add-custom-domain.yaml) to your Microsoft Entra tenant. Your users can sign in with one of these domain names.
...
## Test signing in with one of your users
 ...
 2. Sign in with a user account that was created in your tenant.  You'll need to sign in using the following format: (user@domain.onmicrosoft.com). Use the same password that the user uses to sign in on-premises.
   ...
   ![Screenshot that shows the my apps portal with a signed in users.](~/identity/hybrid/cloud-sync/media/tutorial-single-forest/verify-1.png)
