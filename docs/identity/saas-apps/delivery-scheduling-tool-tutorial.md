...
1. **[Configure Delivery Scheduling Tool SSO](#configure-delivery-scheduling-tool-sso)** - to configure the single sign-on settings on application side.
    1. **[Create Delivery Scheduling Tool test user](#create-delivery-scheduling-tool-test-user)** - to have a counterpart of B.Simon in Delivery Scheduling Tool that is linked to the Microsoft Entra ID representation of user.
