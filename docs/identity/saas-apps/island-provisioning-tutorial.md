...
1. Under the **Mappings** section, select **Synchronize Azure Active Directory Groups to Island**.

1. Review the group attributes that are synchronized from Azure AD to Island in the **Attribute-Mapping** section. The attributes selected as **Matching** properties are used to match the groups in Island for update operations. Select the **Save** button to commit any changes.
...
1. Define the users that you would like to provision to Island by choosing the desired values in **Scope** in the **Settings** section.
