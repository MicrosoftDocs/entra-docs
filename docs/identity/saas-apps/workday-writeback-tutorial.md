...
In this section, you'll configure how writeback attributes flow from Microsoft Entra ID to Workday. 
...
1. If you're using [Workday to Microsoft Entra ID](workday-inbound-cloud-only-tutorial.md) provisioning job, configure it to flow the Workday *StatusHireDate* field directly to *employeeHireDate* attribute in Microsoft Entra ID. 
   ...
   > [!NOTE]
   > If you're storing the employee start date in any other Microsoft Entra ID *extensionAttribute*, you can use that attribute instead of *employeeHireDate* in the expression below.
...
If *telephoneNumber* has the value `+1 (206) 291-8163 x8125`, the expression below returns `2062918163`.
```C#
...
If *telephoneNumber* has the value `+1 (206) 291-8163 x8125`, the expression below returns `1`.
```C#
...
If *telephoneNumber* has the value `+1 (206) 291-8163 x8125`, the expression below returns `8125`.
```C#
...
5. Once the initial sync is completed, it writes a summary report in the **Provisioning** tab, as shown below.
