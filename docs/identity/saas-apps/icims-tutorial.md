...
1. **[Configure Microsoft Entra Application Registration](#configure-azure-ad-sso)** - to enable your users to use this feature.
    ...
    * **[Add permissions to your Entra Application](#add-permissions-to-your-Entra-Application)** - to allows your app to sign in users and read the signed-in users' profiles.
...
### Add credentials to your Entra Application
 ...
 > [!NOTE]  You must contact iCIMS Technical Support at least 30 days in advance of expiration to provide a new secret and avoid a service interruption.
 
 5. Record the Client secret and expiration date to provide to the iCIMS 
 support team.

 > [!NOTE] if your organization does not want to configure a client secret, you can leverage an OpenID Connect integration and utilize front-channel authentication with the implicit grant flow.
...
### Add permissions to your Entra Application
 ...
 > [!NOTE] iCIMS recommends that you grant Admin Consent to User.Read to avoid prompting users to trust the Azure AD Application.
...
3. **Email**: The email of the user. Emails are mutable and only need to match during the first login, at which point the account is bound with an ATS user.  This option is not recommended for mutable email addresses.
 > [!NOTE] iCIMS recommends that you verify the Azure AD user’s email address before accessing iCIMS ATS via Corporate SSO. This prevents linking an account with an invalid email address.
...
### Submit a support ticket for your SSO integration
 ...
 3. When submitting the ticket please provide the following details
	...
	- Provide the **IdP Domain(s)** - Typically, the domain of your organization’s corporate email addresses (e.g., corporate-domain.com is the domain of name@corporate-domain.com.).  Your organization can leverage multiple IdP domains.
     >  [!NOTE]  The domain must be unique within the region (e.g, gmail.com is an invalid domain.)
    ...
    >  [!NOTE] The logo displays as a 20x20 pixel square on your organization’s iCIMS Universal Login page.
