---
title: Configure Dotcom-Monitor for Single sign-on with Microsoft Entra ID
...
# Configure Dotcom-Monitor for Single sign-on with Microsoft Entra ID
...
1. **[Configure Dotcom Monitor SSO](#configure-dotcom-monitor-sso)** - to configure the single sign-on settings on application side.
   1. **[Create Dotcom Monitor test user](#create-dotcom-monitor-test-user)** - to have a counterpart of B.Simon in Dotcom-Monitor that's linked to the Microsoft Entra representation of user.
...
1. Sign in to the [Microsoft Entra admin center](https://entra.microsoft.com) as at least a [Cloud Application Administrator](~/identity/role-based-access-control/permissions-reference.md#cloud-application-administrator).
1. Browse to **Entra ID** > **Enterprise apps** > **Dotcom-Monitor** > **Single sign-on**.
