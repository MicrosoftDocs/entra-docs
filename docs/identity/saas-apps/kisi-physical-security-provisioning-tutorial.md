...
1. In the **Tenant URL** field, input your Kisi Physical Security Tenant URL and Secret Token. Select **Test Connection** to ensure Microsoft Entra ID can connect to Kisi Physical Security. If the connection fails, ensure your Kisi Physical Security account has the required admin permissions and try again.
