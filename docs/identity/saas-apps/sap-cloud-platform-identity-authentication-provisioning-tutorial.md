...
1 Check if there are any attributes on the user required by SAP applications that are being provisioned from SAP Cloud Identity Services. This tutorial assumes two required attributes, `userName` and `emails[type eq "work"].value`.
