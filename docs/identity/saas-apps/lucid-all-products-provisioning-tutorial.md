...
1. In the **Tenant URL** field, input your Lucid (All Products) Tenant URL and Secret Token. Select **Test Connection** to ensure Microsoft Entra ID can connect to Lucid (All Products). If the connection fails, ensure your Lucid (All Products) account has the required admin permissions and try again.
