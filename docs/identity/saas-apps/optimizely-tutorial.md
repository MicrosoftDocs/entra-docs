...
In this article,  you learn how to integrate Optimizely with Microsoft Entra ID. When you integrate Optimizely with Microsoft Entra ID, you can:
...
1. **[Configure Optimizely SSO](#configure-optimizely-sso)** - to configure the single sign-on settings on application side.
    1. **[Create Optimizely test user](#create-optimizely-test-user)** - to have a counterpart of B.Simon in Optimizely that's linked to the Microsoft Entra representation of user.
...
1. On the **Basic SAML Configuration** section, perform the following steps:
	...
	> [!NOTE]
	> These values aren't real. You update these values with the actual Sign-on URL and Identifier which is explained later in the article. You can also refer to the patterns shown in the **Basic SAML Configuration** section.

5. Your Optimizely application expects the SAML assertions in a specific format, which requires you to add custom attribute mappings to your SAML token attributes configuration. The following screenshot shows the list of default attributes. Select **Edit** icon to open **User Attributes** dialog.
