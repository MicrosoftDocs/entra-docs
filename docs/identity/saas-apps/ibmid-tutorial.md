---
title: Configure IBMid for Single sign-on with Microsoft Entra ID
...
# Configure IBMid for Single sign-on with Microsoft Entra ID

In this article,  you learn how to integrate IBMid with Microsoft Entra ID. When you integrate IBMid with Microsoft Entra ID, you can:
...
1. **[Configure IBMid SSO](#configure-ibmid-sso)** - to configure the single sign-on settings on application side.
    1. **[Create IBMid test user](#create-ibmid-test-user)** - to have a counterpart of B.Simon in IBMid that's linked to the Microsoft Entra representation of user.
...
1. Sign in to the [Microsoft Entra admin center](https://entra.microsoft.com) as at least a [Cloud Application Administrator](~/identity/role-based-access-control/permissions-reference.md#cloud-application-administrator).
1. Browse to **Entra ID** > **Enterprise apps** > **IBMid** > **Single sign-on**.
...
In this section, a user called Britta Simon is created in IBMid. IBMid supports just-in-time user provisioning, which is enabled by default. There's no action item for you in this section. If a user doesn't already exist in IBMid, a new one is created after authentication.
