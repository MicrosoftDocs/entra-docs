---
title: Configure EasySSO for Bamboo for Single sign-on with Microsoft Entra ID
...
# Configure EasySSO for Bamboo for Single sign-on with Microsoft Entra ID

In this article,  you learn how to integrate EasySSO for Bamboo with Microsoft Entra ID. When you integrate EasySSO for Bamboo with Microsoft Entra ID, you can:
...
1. **[Configure EasySSO for Bamboo SSO](#configure-easysso-for-bamboo-sso)** - to configure the single sign-on settings on application side.
    1. **[Create EasySSO for Bamboo test user](#create-easysso-for-bamboo-test-user)** - to have a counterpart of B.Simon in EasySSO for Bamboo that's linked to the Microsoft Entra representation of user.
...
1. Sign in to the [Microsoft Entra admin center](https://entra.microsoft.com) as at least a [Cloud Application Administrator](~/identity/role-based-access-control/permissions-reference.md#cloud-application-administrator).
1. Browse to **Entra ID** > **Enterprise apps** > **EasySSO for Bamboo** > **Single sign-on**.
...
1. On the **Set up single sign-on with SAML** page, In the **SAML Signing Certificate** section, select **Download** links for **Certificate (Base64)** or **Federation Metadata XML** options and save either or all to your computer. You need it later to configure Bamboo EasySSO.
...
6. Select **Save** button on the bottom of the page. You see the content of the Metadata or Certificate files is parsed into the configuration fields. EasySSO for Bamboo configuration is complete.
...
However, if you don't wish to enable automatic user provisioning on the user first login, users must exist in backend User Directories the Bamboo instance make use of, such as LDAP or Atlassian Crowd.
...
When you select the EasySSO for Bamboo tile in the My Apps, you should be automatically signed in to the Bamboo instance for which you set up SSO. For more information about the My Apps, see [Introduction to the My Apps](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510).
...
This scenario presumes you have enabled **SAML Login Button** in **Look & Feel** tab in your Bamboo EasySSO configuration page (see above). Open your Bamboo login URL in browser incognito mode to avoid any interference with your existing sessions. Select **SAML Login** button and you get redirected to Microsoft Entra user authentication flow. Once successfully completed you be redirected back to your Bamboo instance as authenticated user via SAML.
...
In this case you have to follow the [instructions on this page](https://techtime.co.nz/display/TECHTIME/EasySSO+How+to+get+the+logs#EasySSOHowtogetthelogs-RETRIEVINGTHELOGS) to get access to **atlassian-bamboo.log** file. Details of the error is available by the reference ID found on EasySSO error page.
