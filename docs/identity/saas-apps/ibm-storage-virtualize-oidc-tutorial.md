---
title: 'Tutorial: Microsoft Entra single sign-on (SSO) integration with IBM Storage Virtualize (OIDC)'
description: Learn how to configure single sign-on between Microsoft Entra and IBM Storage Virtualize (OIDC).
...
# Customer intent: As an IT administrator, I want to learn how to configure single sign-on between Microsoft Entra ID and IBM Storage Virtualize (OIDC) so that I can control who has access to IBM Storage Virtualize (OIDC), enable automatic sign-in with Microsoft Entra accounts, and manage my accounts in one central location.
---

# Tutorial: Microsoft Entra single sign-on (SSO) integration with IBM Storage Virtualize (OIDC)

In this tutorial, you'll learn how to integrate IBM Storage Virtualize (OIDC) with Microsoft Entra ID. When you integrate IBM Storage Virtualize (OIDC) with Microsoft Entra ID, you can:

Use Microsoft Entra ID to control who can access IBM Storage Virtualize (OIDC).
Enable your users to be automatically signed in to IBM Storage Virtualize (OIDC) with their Microsoft Entra accounts.
...
* IBM Storage Virtualize (OIDC) single sign-on (SSO) enabled subscription.

## Add IBM Storage Virtualize (OIDC) from the gallery

To configure the integration of IBM Storage Virtualize (OIDC) into Microsoft Entra ID, you need to add IBM Storage Virtualize (OIDC) from the gallery to your list of managed SaaS apps.

1. Sign in to the [Microsoft Entra admin center](https://entra.microsoft.com) as at least a [Cloud Application Administrator](~/identity/role-based-access-control/permissions-reference.md#cloud-application-administrator).

1. Browse to **Identity > Applications > Enterprise applications > New application**.

1. In the **Add from the gallery** section, enter **IBM Storage Virtualize (OIDC)** in the search box.

1. Select **IBM Storage Virtualize (OIDC)** in the results panel and then add the app. Wait a few seconds while the app is added to your tenant.
...
1. Browse to **Identity** > **Applications** > **Enterprise applications** > **IBM Storage Virtualize (OIDC)** > **Single sign-on**.
...
1. Perform the following steps in the below section:
    ...
    1. Copy **Application (client) ID** and use it later in the IBM Storage Virtualize (OIDC) side configuration.

        [![Screenshot of application client values.](common/application-id.png)](common/application-id.png#lightbox)
        
1. Navigate to **Authentication** tab on the left menu and perform the following steps:

    1. In the **Redirect URIs** textbox, paste the **Redirect URI** value, which you have copied from IBM Storage Virtualize (OIDC) side.
...
1. Navigate to **Certificates & secrets** on the left menu and perform the following steps:
    ...
    1. Once you add a client secret, **Value** will be generated. Copy the value and use it later in the IBM Storage Virtualize (OIDC) side configuration.
...
In this section, you'll enable B.Simon to use single sign-on by granting access to IBM Storage Virtualize (OIDC).

1. Sign in to the [Microsoft Entra admin center](https://entra.microsoft.com) as at least a [Cloud Application Administrator](~/identity/role-based-access-control/permissions-reference.md#cloud-application-administrator).
1. Browse to **Identity** > **Applications** > **Enterprise applications** > **IBM Storage Virtualize (OIDC)**.
...
## Configure IBM Storage Virtualize (OIDC) SSO
