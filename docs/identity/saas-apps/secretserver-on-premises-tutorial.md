...
2. **[Configure Secret Server SSO](#configure-secret-server-sso)** - to configure the single sign-on settings on application side.
    1. **[Create Secret Server test user](#create-secret-server-test-user)** - to have a counterpart of B.Simon in Secret Server that is linked to the Microsoft Entra representation of user.
