...
# Configure iPass SmartConnect for automatic user provisioning with Microsoft Entra ID
...
iPass SmartConnect is available in the following [national cloud deployments](/graph/deployments).
...
1. Review the user attributes that are synchronized from Microsoft Entra ID to iPass SmartConnect in the **Attribute Mapping** section. The attributes selected as **Matching** properties are used to match the user accounts in iPass SmartConnect for update operations. Select the **Save** button to commit any changes.
