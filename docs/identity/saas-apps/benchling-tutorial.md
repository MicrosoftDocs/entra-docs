---
title: Configure Benchling for Single sign-on with Microsoft Entra ID
...
# Configure Benchling for Single sign-on with Microsoft Entra ID

In this article,  you learn how to integrate Benchling with Microsoft Entra ID. When you integrate Benchling with Microsoft Entra ID, you can:
...
1. **[Configure Benchling SSO](#configure-benchling-sso)** - to configure the single sign-on settings on application side.
    1. **[Create Benchling test user](#create-benchling-test-user)** - to have a counterpart of B.Simon in Benchling that's linked to the Microsoft Entra representation of user.
...
1. Sign in to the [Microsoft Entra admin center](https://entra.microsoft.com) as at least a [Cloud Application Administrator](~/identity/role-based-access-control/permissions-reference.md#cloud-application-administrator).
1. Browse to **Entra ID** > **Enterprise apps** > **Benchling** > **Single sign-on**.
...
1. Select **Set additional URLs** and perform the following step if you wish to configure the application in **SP** initiated mode:
	...
	> [!NOTE]
	> These values aren't real. Update these values with the actual Identifier, Reply URL and Sign-on URL. Contact [Benchling Client support team](mailto:support@benchling.com) to get these values. You can also refer to the patterns shown in the **Basic SAML Configuration** section.
...
In this section, a user called B.Simon is created in Benchling. Benchling supports just-in-time user provisioning, which is enabled by default. There's no action item for you in this section. If a user doesn't already exist in Benchling, a new one is created after authentication.
...
* Select **Test this application**, this option redirects to Benchling Sign on URL where you can initiate the login flow.  

* Go to Benchling Sign-on URL directly and initiate the login flow from there.

#### IDP initiated:

* Select **Test this application**, and you should be automatically signed in to the Benchling for which you set up the SSO 

You can also use Microsoft My Apps to test the application in any mode. When you select the Benchling tile in the My Apps, if configured in SP mode you would be redirected to the application sign on page for initiating the login flow and if configured in IDP mode, you should be automatically signed in to the Benchling for which you set up the SSO. For more information about the My Apps, see [Introduction to the My Apps](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510).
