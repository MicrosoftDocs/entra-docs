...
After the user is created, the user's **Authentication Method** selection must be **Active Directory**, the **SAML Single Sign-on** check box must be selected, and the user name from Microsoft Entra ID must be **Active Directory or Certificate ID**.
