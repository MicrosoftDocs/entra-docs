...
This article shows you how to deactivate an enterprise application, view deactivated applications, and reactivate them when necessary using Microsoft Graph API calls.
...
1. List all deactivated applications
    ...
    ```http
    GET https://graph.microsoft.com/v1.0/applications?$filter=isDisabled eq true
    ```

1. Get specific application status

    ```http
    GET https://graph.microsoft.com/v1.0/applications/{application-id}?$select=displayName,isDisabled,appId
    ```

1. List deactivated applications in Enterprise apps (service principals)

    ```http
    GET https://graph.microsoft.com/v1.0/servicePrincipals?$filter=accountEnabled eq false and isDisabled eq true
    ```
