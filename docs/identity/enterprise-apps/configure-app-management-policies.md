...
| audiences                   | Restricts creation or promotion of apps based on signInAudience values. | Prevents unsanctioned multitenant or consumer-facing applications | Can be configured through [app management policy APIs](/graph/api/resources/applicationauthenticationmethodpolicy?view=graph-rest-beta&preserve-view=true) and the [Microsoft Entra admin center](https://aka.ms/app-mgmt-policy-ux). Referred to as `Block multitenant applications` and `Block consumer account applications` in the Microsoft Entra admin center. |
...
- The [Security Administrator](/entra/identity/role-based-access-control/permissions-reference#security-administrator) role, AND the [Cloud App Administrator](/entra/identity/role-based-access-control/permissions-reference#cloud-application-administrator) or [Application Administrator](/entra/identity/role-based-access-control/permissions-reference#application-administrator) role. OR, just the [Global Administrator](~/identity/role-based-access-control/permissions-reference.md#global-administrator) role.
...
This type of exception - sometimes labeled an 'actor' or 'caller' exception - is configured using [custom security attributes](/entra/fundamentals/custom-security-attributes-overview). Because of this, you need two additional roles for this scenario, in addition to the roles from [prerequisites](#prerequisites).
...
Then, [create the definition](/graph/api/directory-post-customsecurityattributedefinitions).
...
Custom security attributes can be assigned to both [users](/entra/identity/users/users-custom-security-attributes) and [service principals](/entra/identity/enterprise-apps/custom-security-attributes-apps) through the Microsoft Entra admin center, but you can also do so using Microsoft Graph. 
...
[Custom policies](/graph/api/resources/appmanagementpolicy?view=graph-rest-beta&preserve-view=true) are applied to specific applications and service principals. They're used to override the tenant-wide configuration for a specific app. You can learn more about that [here](/graph/api/resources/applicationauthenticationmethodpolicy?view=graph-rest-beta&preserve-view=true).
...
- [App management policy API overview](/graph/api/resources/applicationauthenticationmethodpolicy?view=graph-rest-beta&preserve-view=true)
