---  
title: Restrict password usage on Entra apps  
description: Block the addition of passwords on Entra apps to improve security 
author: arcrowe
ms.author: arcrowe
ms.date: 08/19/2025
ms.topic: how-to
...
# Restrict password usage on Entra apps
...
Password addition can also be blocked using the [Entra admin center](https://aka.ms/app-mgmt-policy-ux).  See the [app management policy usage documentation](https://aka.ms/app-mgmt-policy-ux-docs) for more details.

Enabling this setting will block the addition of passwords to existing apps, or setting passwords during new app creation.  You can gauge the impact of enabling this setting by identifying recent password addition activity.  From the Microsoft 365 admin center:
...
Blocking addition of new passwords won't impact existing passwords.  Existing apps using passwords can be identified using the Microsoft 365 admin center.

1. Go to the admin center and select Org settings.
1. Select Restricted Mode, find the **Block addition of new password credentials to apps** setting.
1. Select **download report** to view existing apps with passwords.

Apps using passwords should be modernized before their existing passwords are removed.
