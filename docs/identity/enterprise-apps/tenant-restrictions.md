...
The report might contain limited information, such as target directory ID, when a user who is in a tenant other than the Restricted-Access-Context tenant signs in. In this case, user identifiable information, such as name and user principal name, is masked to protect user data in other tenants (For example, `"{PII Removed}@domain.com" or aaaabbbb-0000-cccc-1111-dddd2222eeee` in place of usernames and object IDs as appropriate).
