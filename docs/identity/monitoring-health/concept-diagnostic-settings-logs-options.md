...
## Priority-based log routing

To help you decide which logs to save, we have categorized the Microsoft Entra logs into categories based on their importance for security investigations and general compliance requirements. We recommend storing at least the high-priority logs for at least two years. 

- **Tier 1**: We highly recommend routing these logs for storage, analysis, or monitoring. These logs provided the greatest insights into identity-based risks, are essential for troubleshooting, and are commonly required for governance and compliance requirements for a variety of industries. 

- **Tier 2**: Organizations that have premium features enabled or a complex tenant architecture might benefit from these logs. 

- **Tier 3**: These logs are either only available if your organization has a specific configuration or provide a large volume of data that isn’t essential for security investigations or troubleshooting. 

## Tier 1

We recommend all customers route the following logs to an endpoint for storage, analysis, or monitoring. If you are limited by licensing or storage costs, prioritize these logs first.
...
These logs are important for security investigations and compliance, but they might not be as critical as the high priority logs. If you have the resources, we recommend routing these logs to an endpoint for storage, analysis, or monitoring.
...
The `ManagedIdentitySignInLogs` provide similar insights as the service principal sign-in logs, but for managed identities, where Azure manages the secrets. For more information, see [Managed identity sign-ins](concept-managed-identity-sign-ins.md).
...
The `MicrosoftServicePrincipalSignInLogs` are set of logs that captures when Microsoft services authenticate into other Microsoft services within a tenant. For example, these logs capture service-to-service authentication when a user opens a Word document inside Microsoft Teams. Previously unavailable due to the size and complexity of the dataset, these logs are now available by configuring diagnostic settings only. We want to emphasize that these are options logs and not essential for most security investigations. We advise against disabling applications or changing configurations based on this data. 