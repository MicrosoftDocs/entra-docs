...
Microsoft Entra ID supports applying sensitivity labels to Microsoft 365 groups when those labels are published in the [Microsoft Purview portal](purview-portal.md) or the [Microsoft Purview compliance portal](purview-compliance-portal.md) and they are configured for groups and sites. Sensitivity labels can be applied to groups across services such as Outlook, Microsoft Teams, and SharePoint. For more information about which apps and services support these, see [Support for sensitivity labels](/purview/sensitivity-labels-teams-groups-sites#support-for-the-sensitivity-labels) from the Purview documentation.
