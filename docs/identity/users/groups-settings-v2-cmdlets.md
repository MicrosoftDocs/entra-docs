...
The cmdlet prompts you for the credentials you want to use to access your directory. In this example, karen@drumkit.onmicrosoft.com is used to access the demonstration directory. The cmdlet returns a confirmation to show the session was connected successfully to your directory:
...
To retrieve existing groups from your directory, use the `Get-MgGroup` cmdlet. 
...
To create a new group in your directory, use the `New-MgGroup` cmdlet. This cmdlet creates a new security group called "DemoGroup":
...
To update an existing group, use the `Update-MgGroup` cmdlet. In this example, the DisplayName property of the group "Intune Administrators" is changed. First, find the group using the `Get-MgGroup` cmdlet and filter using the DisplayName attribute:
...
Next, change the Description property to the new value "Intune Device Administrators":

```powershell
    PS C:\Windows\system32> Update-MgGroup -GroupId 958d212c-14b0-43d0-a052-d0c2bb555b8b -Description "Demo Group Updated"
```

Now, if you find the group again, the Description property is updated to reflect the new value:
...
You can add, retrieve, and remove group members and verify memberships.

### Add members

To add new members to a group, use the `New-MgGroupMember` cmdlet. This command adds a member to the Intune Administrators group used in the previous example:

```powershell
    PS C:\Windows\system32> New-MgGroupMember -GroupId f76cbbb8-0581-4e01-a0d4-133d3ce9197f -DirectoryObjectId a88762b7-ce17-40e9-b417-0add1848eb68
```

The `-GroupId` parameter is the ObjectID of the group. The `-DirectoryObjectId` is the ObjectID of the user to add as a group member.
...
To remove the member previously added to the group, use the `Remove-MgGroupMemberByRef` cmdlet, as shown here:
...
1. If it returns `EnableGroupCreation : True`, then standard users can create groups. To disable this feature:
...
The `-GroupId` parameter is the ObjectID of the group to which you want to add an owner. The `-DirectoryObjectId` is the ObjectID of the user or service principal to add as an owner.
