...
On-premises password writeback is only visible if you downloaded and configure Microsoft Entra Connect.
...
This can happen if you a Microsoft Entra ID license isn't assigned to the administrator performing the operation.
...
| The user doesn't have a Microsoft Entra ID license assigned. | This can happen if a Microsoft Entra ID license isn't assigned to the desired user. To assign a license to the administrator account in question, follow the steps to [Assign, verify, and resolve problems with licenses](~/identity/users/licensing-groups-assign.md#step-1-assign-the-required-licenses). |
