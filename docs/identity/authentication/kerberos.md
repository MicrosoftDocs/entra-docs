...
A hybrid identity refers to a user identity that exists both in an on-premises Active Directory (AD DS) and in Microsoft Entra ID (formerly Azure AD). These identities are synchronized using tools like Microsoft Entra Connect, allowing users to access both cloud-based and on-premises resources with a single set of credentials. This setup enables seamless authentication and single sign-on (SSO) experiences across environments, making it ideal for organizations transitioning to the cloud while maintaining legacy infrastructure. 
...
**Cloud-Only Identity Support**: Cloud-only users can now authenticate to Azure Files using Entra Kerberos, removing the dependency on on-premises domain controllers or Entra Domain Services. This simplifies access management in fully cloud-native environments. 

**Simplified Hybrid Join** Entra Kerberos supports hybrid join scenarios without requiring ADFS or Entra Connect sync. This is ideal for non-persistent virtual desktop infrastructure (VDI), disconnected forests, and Azure Virtual Desktop 

**Secure Ticket Exchange**: Entra Kerberos uses a secure Ticket Granting Ticket (TGT) exchange model.  
...
>[!IMPORTANT]
> The TGT is only recognized by on-premises AD. Therefore, having access to a partial TGT does not provide access to other resources outside of AD.
...
- The cloud TGT contains authorization data only specific to cloud services and is sufficient for accessing resources integrated with Entra Kerberos, such as Azure Files and Azure SQL.

>[!NOTE]
> The Cloud TGT isn't a replacement for the on-premises TGT. It is another ticket that allows access to cloud resources. The on-premises TGT is still required for accessing on-premises resources.
...
- The client contacts an on-prem AD domain controller to exchange it for a full TGT.
- This full TGT is then used to access on-prem resources like SMB shares or SQL servers.

**Access cloud resources**:
The client uses cloud TGT to request service tickets for cloud resources. No interaction with on-prem AD is needed.
...
| Feature | Cloud TGT | On-Prem TGT |
|---|---|---|
| Issuer | Entra ID | On-Prem AD (via exchange) |
| Realm | KERBEROS.MICROSOFTONLINE.COM | On-prem AD domain |
...
### Windows Authentication access to Azure SQL Managed Instance using Entra Kerberos
Kerberos authentication for Microsoft Entra ID enables Windows Authentication access to Azure SQL Managed Instance. Windows Authentication for managed instances empowers customers to move existing services to the cloud while maintaining a seamless user experience and provides the basis for infrastructure modernization.
Detailed information at [Windows Authentication access to Azure SQL Managed Instance using Entra Kerberos](/azure/azure-sql/managed-instance/winauth-azuread-overview?view=azuresql) 

### Use SSO to sign in to on-premises resources by using FIDO2 keys
Entra Kerberos users can sign in to Windows with modern credentials, such as FIDO2 security keys, and then access traditional Active Directory-based resources.  
...
Entra Kerberos authentication hybrid users to access Azure file shares using Kerberos authentication, using Microsoft Entra ID to issue the necessary Kerberos tickets to access the file share with the SMB protocol.  
...
- Entra Kerberos only supports hybrid user identities created in on-premises AD and synchronized to Microsoft Entra ID via Entra Connect.
- Cloud-only user accounts managed solely in Microsoft Entra ID are not supported for Kerberos authentication.
### Operating System and Device Restrictions
- Entra Kerberos is supported on Microsoft Entra joined or hybrid joined Windows 10 (2004+) and Windows 11 devices.
...
- B2B guest users or users from other Microsoft Entra tenants cannot currently authenticate using Entra Kerberos.
### Password Expiration
...
- Kerberos tickets have a size constraint that limits the number of group SIDs (Security Identifiers) that can be included. The default cap is 1010 groups per ticket. If exceeded, only the first 1010 are included, which can cause access failures for users in large organizations.
### MFA Incompatibility for Azure Files Auth
- Microsoft Entra Kerberos authentication for Azure file shares does not support multifactor authentication (MFA).
- Conditional Access policies enforcing MFA must exclude the storage account application or users will experience authentication failures.
...
- Proper synchronization of key on-premises AD user attributes is essential for Entra Kerberos to work, including onPremisesDomainName, onPremisesUserPrincipalName, and onPremisesSamAccountName.
### Single AD Method per Azure Storage Account
- For Azure Files identity-based authentication, only one AD method (Entra Kerberos, on-premises AD DS, or Entra Domain Services) can be enabled per storage account at a time.
- Switching between methods requires disabling the current method first.
### Kerberos Encryption Settings
- Kerberos ticket encryption with Entra Kerberos uses AES-256 exclusively. SMB channel encryption can be configured separately based on requirements.
