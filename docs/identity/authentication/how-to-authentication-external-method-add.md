...
This authentication method will eventually be able to be included in authentication strengths, and those authentication strengths as part of defining a Conditional Access policy requirement; this functionality will come as part of a preview refresh.
...
ClientId | The Entra ID’s client id. This is the identifier generated by the provider to identify Entra ID. In OIDC parlance this is the client_id that external identity provider assigns to Entra ID, which is also a recipient of a token from the external identity provider.
