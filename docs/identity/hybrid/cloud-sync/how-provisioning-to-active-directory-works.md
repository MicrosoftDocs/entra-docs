...
Password writeback, which synchronizes password changes in Microsoft Entra ID to the matched AD account, isn't available for users created in Microsoft Entra. Users will only be able to authenticate to Kerberos-based applications, through passwordless authentication using the AD account that provisioning creates. For more information, see [How cloud-managed users sign in to the application](tutorial-users-groups-provisioning-walkthrough.md#how-cloud-managed-users-sign-in-to-the-application).
