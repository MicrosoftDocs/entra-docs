...
If an object isn't syncing as expected with Microsoft Entra ID, it can be because of several reasons. If you received an error email from Microsoft Entra ID or you see the error in Microsoft Entra Connect Health, read [Troubleshooting errors during synchronization](tshoot-connect-sync-errors.md) instead. But if you're troubleshooting a problem where the object isn't in Microsoft Entra ID, this article is for you. It describes how to find errors in the on-premises component Microsoft Entra Connect synchronization.
...
| Status | Comment |
| --- | --- |
| stopped-* |The run couldn't finish. This might happen, for example, if the remote system is down and can't be contacted. |
...
| completed-\*-warnings |The run finished, but some data isn't in the expected state. If there are errors, this message is only a symptom. Don't investigate warnings until you've addressed errors. |
| success |No issues. |

When you select a row, the bottom of the **Operations** tab is updated to show the details of that run. On the far-left side of this area, there might be a list titled **Step #**. This list appears only if there are multiple domains in your forest and each domain is represented by a step. The domain name can be found under the heading **Partition**. Under the **Synchronization Statistics** heading, you can find more information about the number of changes that were processed. Select the links to get a list of the changed objects. If there are objects with errors, those errors show up under the **Synchronization Errors** heading.

### Errors on the Operations tab
When there are errors, Synchronization Service Manager shows both the object in error and the error itself as links that provide more information.
...
Right-select the **Call Stack Information** box, select **Select All**, and then select **Copy**. Then copy the stack and look at the error in your favorite editor, such as Notepad.
...
If the error doesn't give enough information, it's time to look at the data itself. Select the link with the object identifier and continue troubleshooting the [connector space imported object](#cs-import).
...
You can perform another useful search by selecting the Microsoft Entra Connector. In the **Scope** box, select **Pending Import**, and then select the **Add** check box. This search gives you all synced objects in Microsoft Entra ID that can't be associated with an on-premises object.  
...
The **Old Value** column shows what currently is stored in Connect. The **New Value** column shows what is received from the source system and isn't applied yet. If there is an error on the object, changes are not processed.
...
### Preview
In the lower-left corner of the **Connector Space Object Properties** window is the **Preview** button. Select this button to open the **Preview** page, where you can sync a single object. This page is useful if you're troubleshooting some custom synchronization rules and want to see the effect of a change on a single object. You can select a **Full sync** or a **Delta sync**. You can also select **Generate Preview**, which only keeps the change in memory. Or select **Commit Preview**, which updates the metaverse and stages all changes to target connector spaces.  
...
It's better to start searching from the source Active Directory connector space. But you can also start searching from the metaverse.
...
Go to the [CS Import](#cs-import) attribute list and check which filter is blocking the object from moving to the MV. The **Connector Space** attribute list shows only non-null and non-empty attributes. For example, if **isCriticalSystemObject** doesn't show up in the list, the value of this attribute is null or empty.

<a name='object-not-found-in-the-azure-ad-cs'></a>

### Object not found in the Microsoft Entra CS
If the object isn't present in the connector space of Microsoft Entra ID but is present in the MV, look at the scoping filter of the outbound rules of the corresponding connector space. Determine if the object is filtered out because the [MV attributes](#mv-attributes) don't meet the criteria.

To look at the outbound scoping filter, select the applicable rules for the object by adjusting the following filter. View each rule and look at the corresponding [MV attribute](#mv-attributes) value.
...
If an object isn't syncing, ask the following questions about attribute states in the metaverse:
- Is the attribute **cloudFiltered** present and set to **True**? If so, then it's filtered according to the steps in [attribute-based filtering](how-to-connect-sync-configure-filtering.md#attribute-based-filtering).
- Is the attribute **sourceAnchor** present? If not, do you have an account-resource forest topology? If an object is identified as a linked mailbox (the attribute **msExchRecipientTypeDetails** has the value **2**), the **sourceAnchor** is contributed by the forest with an enabled Active Directory account. Make sure the master account is imported and synced correctly. The master account must be listed among the [connectors](#mv-connectors) for the object.
...
From the **Connectors** tab you can also go to the [connector space object](#connector-space-object-properties). Select a row and select **Properties**.
