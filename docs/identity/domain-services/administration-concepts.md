...
| Issues | RPO | RTO |
|--------|-----|-----|
| Issues caused by data loss or corruption to Domain Services domain controllers, dependent services, an exploit that compromised the domains, or other customer reported incident that requires restoring a domain controller.  | Five days before the occurrence of the event | Two hours to four days, depending on tenant size |
