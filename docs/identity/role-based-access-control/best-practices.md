...
> [!NOTE]
> Availability of the controls in the preceding table depends on your Microsoft Entra license tier. For example, administrative units, custom roles, and Conditional Access require Microsoft Entra ID P1, while Privileged Identity Management, entitlement management, and access reviews require Microsoft Entra ID P2 or Microsoft Entra ID Governance. Continuous Access Evaluation's critical event evaluation is available in all tenants; its Conditional Access policy evaluation requires Microsoft Entra ID P1. To compare what's included in each tier, see [Microsoft Entra plans and pricing](https://www.microsoft.com/security/business/identity-access-management/azure-ad-pricing).
