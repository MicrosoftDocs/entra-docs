...
- Select users and groups
   ...
   - Directory roles
      - Allows administrators to select specific [built-in directory roles](../role-based-access-control/permissions-reference.md) used to determine policy assignment. For example, organizations might create a more restrictive policy on users actively assigned a [privileged role](/role-based-access-control/permissions-reference.md). Other role types aren't supported, including administrative unit-scoped roles and custom roles.
