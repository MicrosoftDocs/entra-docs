...
> [!NOTE]
> Token Protection currently supports native applications only. Browser-based applications aren't supported.
...
**Apple (Preview)**:
- macOS 11.0 or later. Requires the Microsoft Enterprise single-sign-on (SSO) plug-in. Alternatively, you can also use Platform SSO. Only MDM-managed devices are supported.
