...
1. **Ensure that the users have been provisioned into Microsoft Entra ID.** At this point, the users should be present in Microsoft Entra ID with the attributes required by the target applications. <!-- TODO-Psh 1.12 Ensure that Entra ID has the right users for the workers in SuccessFactors and they are populated with the attributes required by SAP cloud directory in 1.1. -->

1. **Ensure there are no unexpected uncorrelated accounts in Microsoft Entra ID.** There will typically be a few users in Microsoft Entra ID which do not correspond to workers in your authoritative system of record source, including the break glass account for emergency administrative access, accounts for IT vendors and business guests. However, they may also be orphan accounts in Microsoft Entra which resemble those of workers, but were not synchronized with a worker record. These orphan accounts can result from former employees who are no longer in the HR system, from matching errors, or data quality issues, such as a person who changed their name or was rehired. <!--TODO-PSh how to ensure there are no users in Entra ID who do not correspond to SuccessFactors workers because they are orphan accounts. Is there Graph PSh script for this? -->
...
1. **If Microsoft Entra is creating users in Windows Server AD, distribute the Windows Server AD initial credentials for newly created users.**  <!-- TODO-credential AD -->
