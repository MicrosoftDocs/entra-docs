...
> [!IMPORTANT]
> Cloud sync documents a related cleanup scenario: when a cloud sync agent has been uninstalled or stopped and its certificate then expires, the agent is permanently removed and can no longer interact with Microsoft services. For more information, see [Agent removal from the portal after uninstall](~/identity/hybrid/cloud-sync/how-to-configure.md#agent-removal-from-the-portal-after-uninstall). For on-premises application provisioning, contact Microsoft support if you need guidance on registration certificate lifetime or recovery procedures.
...
- Check for new versions periodically through the Microsoft Entra admin center or the [provisioning agent release history](provisioning-agent-release-version-history.md).
