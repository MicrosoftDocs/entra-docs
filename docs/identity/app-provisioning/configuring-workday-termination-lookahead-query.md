...
This adjustment ensures the termination data is available earlier for workers in time zones ahead of Pacific Time, so you can accordingly implement time-zone aware terminations by updating the attribute mapping rules in Entra.
...
> [!IMPORTANT] 
> DO NOT use production provisioning job. Perform the configurations described only with test users using  test instance of Workday and Entra ID.
> [!NOTE] 
...
The above expression checks for the presence of `StatusTerminationLastDayOfWork`, which is the attribute that is retrieved as part of the termination lookahead query.
...
:::image type="content" source="media/configuring-workday-termination-lookahead-query/edit-attribute-mapping.png" alt-text="A of the Edit Attribute fields.":::
...
- **If your Workday provisioning job target is Active Directory,** then in the attribute mappings section, update the logic associated with the `accountDisabled` flag to include a check for **Last Day of Work** as shown below.
...
For example, the scenario above is set in the PST time zone (5/28/2025) and the user, Aus Mike Jordan who is the Melbourne time zone, has been terminated and the last day of work is set to 5/28/2025 Melbourne time. If this was a usual provisioning job run (without lookahead query), we would not see this termination event until the end of day PT on 5/28/2025.

## Run provision on demand for the worker

For the above scenario, Provision on Demand for this user ran on 5/28/2025 at 6pm PT.

With the lookahead query, the last day of work for the user was fetched and the `accountEnabled` flag was updated to **false** as shown below.
