...
* your organization has either SAP ECC, SAP S/4HANA, or other SAP applications, and optionally other applications
* you will be using SAP Cloud Identity Services for provisioning and single-sign on to SAP applications other than SAP ECC

This tutorial illustrates an organization which has a Windows Server AD domain, however, Windows Server AD is not required for this topology.
...
1. **Wait for synchronization from Windows Server AD to Microsoft Entra ID to complete.** <!-- 1.8	 Ensure that any sync from WS AD to Entra ID (with any changes you made to users in AD in step 1.4) have completed. 	?		-->

### Update the Microsoft Entra ID user schema

If you do not not use Windows Server AD, then follow the steps in this section to extend the Microsoft Entra ID user schema.
...
1. **Validate a test user can connect to the SAP applications.** <!-- 1.18	Test that a user can connect to SAP cloud directory and is correctly correlated.	myapps; SAP UI -->
