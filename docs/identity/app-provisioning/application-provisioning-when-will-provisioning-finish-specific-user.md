...
The time it takes for a user, group, or group membership to be provisioned varies based on several factors. 
- The more users, groups, and group memberships in scope for provisioning, the longer that it will take for the sync job to complete. For example, a sync job with 10 groups, each with 20K members, will take significantly longer to provision than a sync job with 1 group of 20K users.
...
- If your sync scope is set to "sync all users and groups," the sync engine will evaluate every user, group in the tenant during the initial cycle to determine if the object is in scope for provisioning. The total number of users, groups, and group members present in the source system (Microsoft Entra ID) impacts performance.  
...
- The provisioning service retries certain failure events. Check the progress bar and [provisioning logs](check-status-user-account-provisioning.md) for any failures.
