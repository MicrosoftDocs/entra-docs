...
The time it takes for a user, group, or group membership to be provisioned varies based on several factors including:
- The total number of users, groups, and group memberships in scope for provisioning. For example, a sync job with 10 groups, each with 20K members, will take significantly longer to provision than a sync job with 1 group of 20K users.  
...
- The total number of users, groups, and group members present in the source system (Microsoft Entra ID). If your sync scope is set to "sync all users and groups," the sync engine will evaluate every user, group in the tenant during the initial cycle to determine if the object is in scope for provisioning. 
- The number of changes such as users created, group membership changes, etc. in the source system impacts the time to process changes during an incremental cycle.  
