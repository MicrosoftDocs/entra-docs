...
1. In the **Advanced settings** section, you can choose the following:
    ...
    - **Access Review Agent**: Select this checkbox to make the access review compatible with the access review agent, which allows reviewers to complete access reviews within Microsoft Teams with the help of informed decisions based on environment context.
    > [!NOTE]
    > The Access Review Agent is only supported with certain types of Access reviews, and require specific permissions. For more information on using the Access Review Agent, see: [Access Review Agent with Microsoft Security Copilot](access-review-agent.md).
