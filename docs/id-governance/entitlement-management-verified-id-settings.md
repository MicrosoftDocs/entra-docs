...
1. Step1 allows the selection of issuers under two types - "Select the type of issuer".
    1. Microsoft Entra Verified ID Network - this could be any Entra Verified ID issuer from the drop down list
...
1. For "Security Partners (Third Party providers)" selection in Step1, If the admin is selecting the issuer for the first time, they will get a link to Security Store to purchase the offer.
...
1. If you want users to complete a Face Check, select **Require Face Check**. This will ask users requesting the access package to perform a real-time, privacy compliant selfie check against the photo that is stored on their Verified ID. Once you select the checkbox, it will ask you to select the claim name that maps to the photo on the ID. For more information on Face Check, see: [Using Face Check with Microsoft Entra Verified ID and unlocking high assurance verifications at scale](../verified-id/using-facecheck.md).
