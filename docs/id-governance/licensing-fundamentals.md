...
- **Agent 365** - Agent 365 provides identity governance capabilities for agent identities. It includes features for governing agent lifecycles and entitlement management for agents, such as assigning agents and service principals to access packages, enabling owners and sponsors to request access on behalf of agents, managing API permissions in access packages, and agent sponsorship tasks in Lifecycle Workflows. For more information, see [governing agent identities (preview)](agent-id-governance-overview.md#license-requirements).
...
- A subscription to **Agent 365** requires that the tenant also have an active subscription to another product, one that contains the `AAD_PREMIUM` service plan. Examples of products meeting this prerequisite include **Microsoft Entra ID P1** or **Microsoft 365 E3**.
